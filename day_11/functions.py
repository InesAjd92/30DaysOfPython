...
def check_season(month): 
        ...
        elif month in ["June", "May", "April"]:
            print("This is spring")
        elif month in ["September", "August", "July"]:
...
print(calculate_median([1, 3, 3, 1, 3, 4]))
...
def calculate_mode(lst):
    ...
    for elm in stocked_data:
        if stocked_data[elm] > max_count:
            max_count = stocked_data[elm]
            mode = elm
...
def calculate_range(lst):
    sorted_lst = sorted(lst)
    highest = sorted_lst[-1]
    lowest = sorted_lst[0]
    range_calc = highest - lowest
    return range_calc
...
def calculate_variance(lst): 
    ...
    mean = total_values / len(lst) 
    # substrac the mean for each data value
    for n in lst: 
        susbtracted_values.append((n - mean) ** 2)  # ajoute chaque carré d'écart à la liste
        variance_cal = (sum(susbtracted_values)) / (len(lst) - 1)
    return variance_cal
    
print(calculate_variance([1, 2, 3, 4, 5]))  # Affiche 2.5
...
def calculate_std(lst): 
    ...
    mean = count / len(lst)
    for n in lst: 
        sum_squared += (n - mean)**2
    variance = sum_squared / (len(lst)-1)
    standard_dev = (variance)**0.5
    return standard_dev

print(calculate_std([1,3,9,3,4,7]))
...
def is_unique(lst):
    ...
    if len(test_set) < len(lst):
        print("The list does not contain only unique items")
    else:
        print("The list contains unique items only")
...
print(is_valid_variable("_var"))       # True
print(is_valid_variable("1variable"))  # False (commence par un chiffre)
print(is_valid_variable("for"))        # False (mot-clé réservé)
print(is_valid_variable("var!able"))   # False (caractère interdit)
